...
export function openProject(projectName, toDos) {
  const toDoGrid = document.querySelector("#todo-grid");
  toDoGrid.innerHTML = "";
  const index = toDos.findIndex((item) => item.projectName === projectName);
  if (toDos[index]) {
    const pageHeader = document.querySelector("#todo-grid-title");
    pageHeader.textContent = toDos[index].projectName;
    toDos[index].toDoArray.map((toDo) => {
      let gridItem = addToDoCard(toDo, toDos);
      toDoGrid.appendChild(gridItem);
    });
...
export function addToDoCard(toDo, toDoArray) {
  ...
  deleteButton.addEventListener("click", () => {
    ...
    const index = toDoArray.findIndex(
      (item) => item.projectName === toDo.projectName
    );
    console.log(index);
    const toDoIndex = toDoArray[index].toDoArray.findIndex(
      (item) => item.toDoTitle == toDo.toDoTitle
    );
